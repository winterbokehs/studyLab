...
public class InterceptorConfig implements WebMvcConfigurer {
    ...
    public void addInterceptors(InterceptorRegistry registry) {
        registry.addInterceptor(new JWTInterceptor())
                ...
                .excludePathPatterns("/userBlog/**")//放行
...
public class JWTUtil {
    ...
    public static String getToken(Map<String,String> map) {
        ...
        calendar.add(Calendar.DATE,1);//指定令牌的过期时间，1天
...
import com.example.myblog.pojo.ResponseEntity;
...
public class JWTInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        //对前端的访问进行跨域开放，虽然配置了全局跨域，但是前端的请求并不知道，所以要再进行一次放行
...
public class shiroConfig {
    ...
    public DefaultWebSecurityManager getDefaultWebSecurityManager(Realm realm){
     ...
     //创建shiroFilter    拦截所有请求    虽然这个没有用但是要写
    @Bean
...
//        map.put("/blog/**","authc");//请求这些页面需要授权和认证
...
```java
package com.example.myblog.realm;

import com.baomidou.mybatisplus.core.conditions.query.QueryWrapper;
import com.example.myblog.mapper.AdminMapper;
import com.example.myblog.pojo.Admin;
...
public class AdminRealm extends AuthorizingRealm {
    ...
    @Autowired
    AdminMapper adminMapper;//取数据库中的数据
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken token) throws AuthenticationException {
        ...
        log.info("数据库用户为：{}", DBadmin);
        if (DBadmin != null) {
            return new SimpleAuthenticationInfo(DBadmin.getUsername(), DBadmin.getPassword(),
                    ByteSource.Util.bytes(DBadmin.getSalt()), this.getName());
        }
        return null;
    }
}
...
```java
 /**
    ...
    public ResponseEntity login(@RequestBody Admin admin) {
        ...
        log.info("---------登录的用户为：=====>{}", admin);

        try {
            //login方法调用自定义的realm方法进行认证
            subject.login(new UsernamePasswordToken(admin.getUsername(), admin.getPassword()));
            log.info("-----匹配成功-----欢迎您，[{}]", admin.getUsername());

            //异步更新登录时间 ， 先返回结果再执行该语句
            adminService.updateLast_Login(admin.getUsername());

            Admin DBadmin = adminService.findAdmin(admin.getUsername());


            //shiro验证用户名和密码   成功后进行jwt  Token的添加
            Map<String, String> payload = new HashMap<>();
            payload.put("username",DBadmin.getUsername());
            payload.put("password",DBadmin.getPassword());
            //生成令牌
            ...
            //返回最后登录的时间和生成的token
            map.put("token",token);
            map.put("lastLogin",simpleDateFormat.format(DBadmin.getLastlogin()));
...
3. 后台会进行 `token` 的校验，如果有误，会直接返回 401。
